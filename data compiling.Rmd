...
demo_all = merge(demo_chol, diet, by = "seqn") 
...
demo_x = demo_all %>% 
 distinct(seqn, .keep_all = TRUE) %>% 
  mutate(race = as.factor(race),
         ...
         ever_cancer = as.factor(ever_cancer),
         type_of_cancer = as.factor(type_of_cancer),
         hbp = as.factor(hbp),
         h_chol = as.factor(h_chol),
         ) %>% 
  filter(ever_cancer == "yes",
         type_of_cancer %in% c("colon", "rectal")) %>% 
...
```{r}
fit_logistic = demo_x %>% 
  glm(ever_cancer ~ gender + race + diabetes   + chd + heart_attack + stroke + thyroid_prob + liver_cond , data = ., family = binomial())
